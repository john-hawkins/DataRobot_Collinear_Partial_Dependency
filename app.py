...
def get_validation_column(proj):
   #if proj.partition['cv_method'] == 'datetime':
   #   return 'Backtest 1'
   return 'validation'
...
def configure():
    ...
    projs = dr.Project.list()
    test_partition = get_validation_column( proj )
    print("Project: ", proj.id)
    print("Project type: ", proj_type)
    print("Test Partition: ", test_partition)

    for k, v in mods[0].metrics[proj.metric].items():
        print ("KEY:", k, " VALUE:", v)

    return render_template("configure.html", project=proj, models=mods, 
                            projects=projs, test_partition=test_partition, features=useable_feats)
...
def generate():
    if request.method == 'POST':
        ...
        plotpath = "static/" + project_id + "-" + model_id + "-" + diffcol + "-" + colone + "-" + coltwo + ".png"
        ...
        if my_file.is_file():
            ...
            return render_template("generated.html", project=proj, model=mod, diffcol=diffcol,
